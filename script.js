...
const sendComment = () => {
    ...
    // после добавления коммента кнопка снова становиться неактивной, так как все поля пустые
    switchButton();
}
...
// Использовал событие keydown, потому что при написаном тексте можно было зажать Enter и срабатывал до тех пор пока его не отпустят
...
2. доавил класс avtive и накинул на него :hover потому что hover отрабатывал на неактивную кнопку.
*/
const switchButton = () => {
    if (inputName.value.trim() !== '' && inputText.value.trim() !== '') {

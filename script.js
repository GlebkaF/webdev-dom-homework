...
function newComment() {
    ...
    fetch("https://webdev-hw-api.vercel.app/api/v1/ilia-abramov/comments", {
        method: "POST",
        body: JSON.stringify({
            text: textareaCommentElement.value,
        ...
        response.json().then((responseDate) => {
            ...
            <li class="comments__list">
                ...
                <div class="comments__footer">
                    ...
                    <div>
                        <span class="likes">${listObject.likes}</span>
                        <button data-hard="${i}" class="comments__button-hard"></button>
                    </div>
                </div>
            </li>`
            ).join("");
            
            formElements[0].classList.remove("-display-none");
            loaderElements[0].classList.add("-display-none")
            ...
            for (const buttonHardElement of buttonHardElements) {
                buttonHardElement.addEventListener("click", () => {
                    ...
                    const buttonLikes = document.querySelectorAll(".comments__button-hard");
                    if (!listObject[i].isLiked) {
                        listObject[i].isLiked = true;
                        listObject[i].likes += 1;
                        likes[i].innerHTML = listObject[i].likes;
                        buttonLikes[i].classList.add("-active-like");
                    } else {
                        listObject[i].isLiked = false;
                        listObject[i].likes -= 1;
                        likes[i].innerHTML = listObject[i].likes;
                        buttonLikes[i].classList.remove("-active-like");
                    }
                });
            }
